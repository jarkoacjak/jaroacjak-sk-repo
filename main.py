...
def main():
    ...
    if not params:
        xbmcplugin.addDirectoryItem(handle, build_url({'mode': 'list_sk'}), xbmcgui.ListItem(label="[B]🇸🇰 SLOVENSKÉ STANICE[/B]"), True)
    ...
    # --- 2. SLOVENSKÉ STANICE ---
        ...
        # EPG pre JOJ (podľa obrázka)
        epg_joj = [{"time": "12:15", "title": "James Bond: Casino Royale"}, {"time": "15:25", "title": "Hviezdy nad hlavou 9"}, {"time": "16:45", "title": "Na chalupe"}, {"time": "17:50", "title": "Nové bývanie"}, {"time": "19:00", "title": "Krimi"}, {"time": "19:30", "title": "Noviny TV JOJ"}]
        current_joj = "Sledujte TV JOJ"
        for item in epg_joj:
            start = datetime.strptime(item["time"], "%H:%M").replace(year=now.year, month=now.month, day=now.day)
            if now >= start: current_joj = item["title"]
    ...
    # --- 3. ČESKÉ STANICE ---
        ...
        for s in cz_stanice:
            url = "https://live.cdn.joj.sk/live/andromeda/" + s["u"] + joj_headers
