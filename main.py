...
def main():
    ...
    # --- KOMPLETNÝ PROGRAM PODĽA OBRÁZKA ---
    epg_joj = [
    ...
    if not params:
        ...
        # Živé vysielanie - EPG iba pre JOJ
        current_show = ""
        for item in epg_joj:
        ...
        for i in range(7):
            ...
            xbmcplugin.addDirectoryItem(handle, build_url({'mode': 'archive_list', 'offset': str(i)}), xbmcgui.ListItem(label=label), True)
        xbmcplugin.endOfDirectory(handle)
        ...
        for r in epg_joj:
            ...
            # Pridať do zoznamu iba ak už relácia reálne skončila
            if now > (start_dt + timedelta(minutes=r["dur"])):
                ...
                li.setInfo('video', {'title': r['title'], 'plot': "Archívna nahrávka"})
                li.setProperty('IsPlayable', 'true')
                xbmcplugin.addDirectoryItem(handle, url, li, False)
                items_added += 1
        
        if items_added == 0:
            xbmcplugin.addDirectoryItem(handle, "", xbmcgui.ListItem(label="Zatiaľ žiadne relácie v archíve"), False)
